    ...
    private val dataSourceFactory =
        ResolvingDataSource.Factory(
            ...
            database.query {
                ...
                // It belongs solely to onDownloadChanged()'s STATE_COMPLETED branch, which
                // only fires once the download has actually finished.
                val existing = getSongByIdBlocking(mediaId)?.song
                val thumbnailUrl = playbackData.videoDetails?.thumbnail?.thumbnails?.lastOrNull()?.url
                val updatedSong = existing?.let {
                    // Back-fill artwork on pre-existing rows so downloaded songs always
                    // have cover art, even if the song was first cached without a thumbnail.
                    if (it.thumbnailUrl.isNullOrBlank() && !thumbnailUrl.isNullOrBlank()) {
